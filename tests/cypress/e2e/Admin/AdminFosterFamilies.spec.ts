...
describe('Foster Families search records and add Family', () => {
    ...
    it('Allows an LA user to navigate; to the Foster Families search page and create a new Foster Family record', () => {
        ...
        cy.get('h1').should('include.text', 'Manage foster families');
        cy.url().should('include', 'FosterFamilies/Search');
        //Child link test
        cy.get('table tbody tr').first().find('td').eq(0).find('a').click();
        cy.get('h1').should('include.text', 'Eligibility code for');
        cy.url().should('include', '/FosterFamilies/Code');
        cy.visit('FosterFamilies/Search');
        //Carer link test
        cy.get('table tbody tr').first().find('td').eq(3).find('a').click();
        cy.get('h1').should('include.text', 'Foster family of');
        cy.url().should('include', '/FosterFamilies/Family');
        cy.visit('FosterFamilies/Search');
